 ...
 */
    ...
    $tableHeaders = $this->Html->tableHeaders(array('',
        $this->Paginator->sort('EventType.name', 'Nombre', array('escape' => false)),
        $this->Paginator->sort('EventType.color', 'Color', array('escape' => false)),
	$this->Paginator->sort('Event.created', '<i class="icon-time"></i> Fecha Registro', array('escape' => false)),
	$this->Paginator->sort('Event.fecha_update', '<i class="icon-time"></i> Última Actualización', array('escape' => false)),
	$this->Paginator->sort('Event.status', 'Status', array('escape' => false)),
		    'Acci&oacute;n'),
...
<div id="breadcrumb">
      <?php
              ...
              $this->Html->addCrumb('Tipo de Eventos',  '/full_calendar/EventTypes'); 
              $this->Html->addCrumb('Consulta' ,  '/full_calendar/EventTypes' , array('class' => 'breadcrumblast')); 
 ...
 <div class="row-fluid">
  <div class="span12 actions">
   <ul class="nav-buttons">
    <li>
        <?php
         echo $this->Html->link('New Tipo de Evento', 
                                array('plugin'=>'full_calendar','controller'=>'EventTypes','action'=>'add'),
 ...
 <div class="row-fluid">
  <div class="span12">
   <div class="blocks filter row-fluid" style="text-align:right;">
    <?php echo $this->Form->create('EventType',array('action'=>'index','class'=>'form-inline'));  ?>
...
<?php
 ...
 ?>
   ...
   <table id="example-datatables3" class="table table-striped table-bordered table-hover">
    ...
    <?php $p=1; foreach ($eventTypes as $cliente):  ?>
    <tr>
     <td><?php echo $p; ?></td>
     <td><?php echo $cliente['EventType']['name']; ?> </td>
     <td><?php echo $cliente['EventType']['color'];  ?></td>
     <td><?php echo $cliente['EventType']['created']; ?></td>
     <td><?php echo $cliente['EventType']['fecha_update']; ?></td>
     <td>
      <?php
       if ($cliente['EventType']['ver']==0) $sclass="icon-remove red ajax-toggle";
       else $sclass="icon-ok green ajax-toggle";

         echo $this->Html->link('<i class="'.$sclass.'"></i>',
                                array('plugin'=>'full_calendar','controller'=>'EventTypes','action'=>'status',$cliente['EventType']['id']),
  ...
  <?php
         echo $this->Html->link('<i class="icon-search icon-large"></i>',
                                array('action'=>'view',$cliente['EventType']['id']),
				array('title'=>'Ver este elemento',
				      'escape' => false,
				      'class'=>'view',
				      'data-title'=>'Ver este elemento',
         ...
         echo $this->Html->link('<i class="icon-pencil icon-large"></i>',
                                array('action'=>'edit',$cliente['EventType']['id']),
